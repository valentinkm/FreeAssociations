...
generalizability.py
───────────────────
Core analysis functions for testing model generalization and comparing models.
This includes SPP and 3TT analysis, as well as model alignment and performance comparisons.
"""
import pandas as pd
import numpy as np
import json
...
def analyze_holistic_similarity(spp_data_to_analyze: pd.DataFrame, lexicon_path: Path):
    ...
    # <<< FIX: Fully implemented the analysis logic >>>
    results = []
...
def compare_models_on_task(task: str):
    ...
    models_to_compare = ["gpt-4o", "gemini-1.5-flash-latest", "llama3"]
    nsets_for_comparison = 25
    
    print(f"Loading task data for {task.upper()}...")
    if task == 'spp':
        task_data, _ = data_utils.get_spp_data_for_analysis(num_pairs_to_probe=500)
    else:
        task_data, _ = data_utils.get_3tt_data_for_analysis(num_triplets_to_probe=500)
    ...
    for model_name in models_to_compare:
        ...
        if not lexicon_path.exists():
            print(f"⚠️ Warning: Lexicon file not found for '{model_name}'. Skipping.")
            continue
    ...
    plt.figure(figsize=(10, 6))
    sns.barplot(data=results_df, x="Model", y="Score", hue="Model", palette="viridis", legend=False)
    plt.title(f"Model Performance Comparison on {task.upper()} Task (n_sets={nsets_for_comparison})", fontsize=16)
...
def compare_model_alignment(models: list, nsets: int, ncues: int):
    ...
    for model_name in models_to_compare:
        ...
        if lexicon_path.exists():
            with open(lexicon_path, 'r') as f:
                processed_words = {json.loads(line)['word'].lower() for line in f}
        
        print(f"Found {len(processed_words)} existing cues in '{lexicon_path.name}'.")
        num_needed = ncues - len(processed_words)
        if num_needed > 0:
            print(f"Need to generate {num_needed} more cues.")
            available = [c for c in all_human_cues if c not in processed_words]
            if not available:
                print("⚠️ No more unique cues to generate.")
                continue
            
            cues_to_generate = random.sample(available, k=min(num_needed, len(available)))
            llm_generation.generate_lexicon_data(set(cues_to_generate), model_name, lexicon_path, nsets, settings.GENERALIZE_DEFAULTS['prompt'])
        else:
            print("✅ Lexicon is sufficient.")
    ...
    for model_name in models_to_compare:
        ...
        comparison_cues = random.sample(available_cues, k=min(ncues, len(available_cues)))
        alignment_scores = [_wjacc(human_norms.get(word.lower(), Counter()), llm_lexicon[word]) for word in comparison_cues if word.lower() in human_norms]
    ...
    results_df = pd.DataFrame(results)
    plt.figure(figsize=(10, 6))
    sns.barplot(data=results_df, x="Model", y="Alignment Score", hue="Model", palette="viridis", legend=False)
    plt.title(f"Model Alignment vs. Human Norms (n_sets={nsets}, n_cues={ncues})", fontsize=16)
    plt.ylabel("Average Alignment (Weighted Jaccard)")
    plt.xlabel("Model")
    plt.xticks(rotation=15)
    plt.tight_layout()
